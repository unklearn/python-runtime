...
async def read_and_display(socketio, payload, filename, *cmd):
    ...
    try:
        ...
        socketio.emit(CellEvents.END_RUN, {
            'id': cell_id,
            'status': status
        })
    return rc
...
def capture_logs(socketio, payload, filename, *cmd):
    ...
    if loop.is_running():
        ...
        asyncio.run_coroutine_threadsafe(read_and_display(socketio, payload, filename, *cmd), loop)
    else:
        loop.run_until_complete(read_and_display(socketio, payload, filename, *cmd))
